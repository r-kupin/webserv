...
TEST_F(NestedConfigLocationSearchTest, SuperNested) {
    ...
    const LocSearchResult &result = FindLocation(uri_);
    EXPECT_EQ(result.location_->address_, "/loc_3");
    EXPECT_EQ(result.location_->full_address_, uri_);
    EXPECT_FALSE(result.location_->ghost_);
    EXPECT_EQ(result.location_->parent_->address_, "/loc_1");
    EXPECT_EQ(result.location_->parent_->parent_->address_, "/");
    EXPECT_EQ(result.status_, "found");
    EXPECT_EQ(result.leftower_uri_, "");
}
...
TEST_F(NestedConfigLocationSearchTest, OneStepNested) {
    ...
    EXPECT_EQ(result.location_->parent_->parent_->address_, "/");
    EXPECT_EQ(result.location_->index_.front(), "index.html");
    EXPECT_EQ(result.status_, "found");
    EXPECT_EQ(result.leftower_uri_, "");
}
...
TEST_F(NestedConfigLocationSearchTest, LocationIsDefinedInServerContext) {
    ...
    const LocSearchResult &result = FindLocation(uri_);
    EXPECT_EQ(result.location_->address_, uri_);
    EXPECT_TRUE(result.location_->ghost_);
    EXPECT_EQ(result.location_->parent_->address_, "/");
    EXPECT_EQ(result.status_, "found");
    EXPECT_EQ(result.leftower_uri_, "");
}
